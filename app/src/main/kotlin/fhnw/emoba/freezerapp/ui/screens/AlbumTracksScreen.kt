...
@Composable
fun AlbumTracksScreen(model: FreezerModel) {
    FreezerTheme(model.isDarkTheme) {
        Scaffold(content = {
            Column() {
                BackBar(model)
                ScreenTitle(title = "Tracks")
                AlbumTracksBody(model = model)
            }
        })
    }
}
...
@Composable
fun AlbumTracksBody(model: FreezerModel) {
    with(model) {
        when {
            isLoading -> {
                ...
                LazyColumn(modifier = Modifier.padding(start = 15.dp, end = 15.dp)) {
                    items(albumsTracksList) { track -> TrackItem(track, model) }
                }
            }
        }
    }
